...
// ---- Standalone mode: ESP32 creates its OWN WiFi network (SoftAP) ----
// No home WiFi, no laptop, no internet required at all. User connects their
// phone directly to this network to reach the dashboard.
const char* ap_ssid = "VP-Farm-zone1";
const char* ap_pass = "vpfarm1234";      // must be >=8 characters
IPAddress apIP(192,168,4,1);
DNSServer dns;
const byte DNS_PORT = 53;

// ---- Crop this zone is growing ----
// Now CONFIGURABLE at runtime via the dashboard (see /api/config below),
// instead of a fixed constant. Persisted to LittleFS so it survives reboots.
String currentCrop = "tomato";   // fallback default if no config file exists yet
const char* CROP_CONFIG_PATH = "/crop.txt";
const char* KNOWN_CROPS[] = {"tomato", "chili", "okra"};
const int   KNOWN_CROPS_COUNT = 3;
const double CONFIDENCE_THRESHOLD = 0.65;   // same threshold as the old laptop service

bool isKnownCrop(const String& crop){
  for (int i = 0; i < KNOWN_CROPS_COUNT; i++){
    if (crop == KNOWN_CROPS[i]) return true;
  }
  return false;
}

void loadCropConfig(){
  if (LittleFS.exists(CROP_CONFIG_PATH)){
    File f = LittleFS.open(CROP_CONFIG_PATH, "r");
    String saved = f.readString();
    f.close();
    saved.trim();
    if (isKnownCrop(saved)){
      currentCrop = saved;
      Serial.println("Loaded saved crop: " + currentCrop);
      return;
    }
  }
  Serial.println("No valid saved crop config, using default: " + currentCrop);
}

void saveCropConfig(const String& crop){
  File f = LittleFS.open(CROP_CONFIG_PATH, "w");
  f.print(crop);
  f.close();
  currentCrop = crop;
  Serial.println("Saved new crop: " + currentCrop);
}
...
String liveSource = "rules_local";   // "model_onboard" | "rules_local" (fail-safe rule, on-device)
...
void predictLocalModel(int soil, float tempC, float humidity){
  ...
  bool knownCrop = isKnownCrop(currentCrop);
  if (!knownCrop){
    liveSource = "rules_local";
    liveConfidence = -1;
    classifyLocal(soil);
  ...
  double confidenceWater = predictWaterProbability(soil, humidity, tempC, currentCrop.c_str());
  ...
  liveReason = "soil=" + String(soil) + "%, P(water)=" + String(confidenceWater,2) +
               ", crop=" + currentCrop + " [on-device model]";
}
...
void handleData(){
  ...
  j += "\"crop\":\""+currentCrop+"\",";          // NEW: which crop this decision was made for
...
// GET  /api/config -> current crop + list of crops the model supports
// POST /api/config  {"crop":"chili"} -> saves selection to LittleFS, applied immediately
void handleConfig(){
  if (server.method() == HTTP_GET){
    String j = "{\"crop\":\""+currentCrop+"\",\"known_crops\":[";
    for (int i = 0; i < KNOWN_CROPS_COUNT; i++){
      if (i) j += ",";
      j += "\""+String(KNOWN_CROPS[i])+"\"";
    }
    j += "]}";
    server.send(200, "application/json", j);
    return;
  ...
  // POST: expects a tiny body like {"crop":"chili"} — hand-parsed, no JSON library needed
  String body = server.arg("plain");
  int start = body.indexOf(':') + 1;
  int q1 = body.indexOf('"', start);
  int q2 = body.indexOf('"', q1 + 1);
  if (q1 < 0 || q2 < 0){
    server.send(400, "application/json", "{\"error\":\"expected {\\\"crop\\\":\\\"tomato\\\"}\"}");
    return;
  }
  String requestedCrop = body.substring(q1 + 1, q2);

  if (!isKnownCrop(requestedCrop)){
    server.send(400, "application/json", "{\"error\":\"unknown crop\"}");
    return;
  }

  saveCropConfig(requestedCrop);
  server.send(200, "application/json", "{\"crop\":\""+currentCrop+"\",\"saved\":true}");
}
...
void setup(){
  ...
  loadCropConfig();   // read saved crop selection (or fall back to default "tomato")

  // ---- SoftAP mode: ESP32 creates its own network, no home WiFi needed ----
  WiFi.mode(WIFI_AP);
  WiFi.softAPConfig(apIP, apIP, IPAddress(255,255,255,0));
  WiFi.softAP(ap_ssid, ap_pass);
  Serial.print("AP IP: "); Serial.println(WiFi.softAPIP());

  dns.start(DNS_PORT, "*", apIP);   // captive portal: any domain resolves to us
  ...
  server.serveStatic("/", LittleFS, "/");   // must come AFTER the "/" handler above

  server.onNotFound([]() {    // captive portal: unknown paths redirect back to dashboard
...
void loop(){
  dns.processNextRequest();   // required for captive portal to work
  server.handleClient();
